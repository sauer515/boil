...
export default function calculateCriticalPath(events: EventInput[]): EventData[] {
    ...
    const resultEvents = Array.from(eventMap.values()).filter(
        (event) => event.name !== 'start' && event.name !== 'end'
    );
